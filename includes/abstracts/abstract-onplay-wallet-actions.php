...
abstract class WooWalletAction extends WC_Settings_API {
	 ...
	 * @var string
	 */
	public $plugin_id = 'onplay_wallet_';
	...
	public function get_action_title() {
		return apply_filters( 'onplay_wallet_gateway_action_title', $this->action_title, $this );
	}
	...
	public function get_action_description() {
		return apply_filters( 'onplay_wallet_action_description', $this->description, $this );
	}
	/**
	 * Check if is enabled.
	 */
	...
	public function admin_options() {
		if ( $this->get_post_data() ) {
			if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'wallet-action-settings' ) ) {
				...
				add_settings_error( $this->id, '200', __( 'Your settings have been saved.', 'onplay-wallet' ), 'updated' );
			} else {
				add_settings_error( $this->id, '200', __( 'Cheatin&#8217; huh?', 'onplay-wallet' ), 'error' );
			}
		}
		echo '<h2>' . esc_html( $this->get_action_title() );
		wc_back_link( __( 'Return to actions', 'onplay-wallet' ), admin_url( 'admin.php?page=onplay-wallet-actions' ) );
		settings_errors();
		echo '</h2>';
