...
if ( ! class_exists( 'Onplay_Wallet_Dokan' ) ) {
	/**
	 * Onplay_Wallet_Dokan Class.
	 *
	 * @since 1.1.10
	 */
	class Onplay_Wallet_Dokan {

		/**
		 * The single instance of the class.
		 *
		 * @var Onplay_Wallet_Dokan
		 * @since 1.1.10
		 */
		 ...
		 * @since 1.2.3
		 */
		private static $method_id = 'onplay_wallet';
		...
		public function __construct() {
			...
			add_filter( 'process_onplay_wallet_general_cashback', array( $this, 'process_onplay_wallet_cashback_callback' ), 10, 2 );
			add_filter( 'process_onplay_wallet_coupon_cashback', array( $this, 'process_onplay_wallet_cashback_callback' ), 10, 2 );
		}
		/**
		...
		public function dokan_settings_fields( $settings_fields ) {
			...
			if ( array_key_exists( self::$method_id, $withdraw_methods ) && ! empty( $withdraw_methods[ self::$method_id ] ) ) {
				$settings_fields['dokan_withdraw']['auto_approve_wallet_withdraw'] = array(
					...
					'label'   => __( 'Wallet Withdraw', 'onplay-wallet' ),
					'desc'    => __( 'Auto approve wallet withdraw on request', 'onplay-wallet' ),
					'type'    => 'switcher',
					'default' => 'off',
				);
			}
		...
		public function dokan_withdraw_updated_callback( $withdraw ) {
			...
			$wallet_transaction = $wpdb->get_row( $wpdb->prepare( "SELECT transactions.transaction_id FROM {$wpdb->base_prefix}onplay_wallet_transactions AS transactions INNER JOIN {$wpdb->base_prefix}onplay_wallet_transaction_meta AS transaction_meta ON transactions.transaction_id = transaction_meta.transaction_id WHERE transaction_meta.meta_key = %s AND transaction_meta.meta_value = %d", '_dokan_withdrawal_id', $withdraw->get_id() ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			if ( 1 === $withdraw->get_status() ) {
				if ( $wallet_transaction && isset( $wallet_transaction->transaction_id ) ) {
					...
					$transaction_id = onplay_wallet()->wallet->credit( $withdraw->get_user_id(), $withdraw->get_amount(), __( 'Withdrawal request #', 'onplay-wallet' ) . $withdraw->get_id() );
		...
		public function dokan_settings_selling_option_vendor_capability( $vendor_capability ) {
			$vendor_capability['product_cahback_change'] = array(
				...
				'label'   => __( 'Cashback Option', 'onplay-wallet' ),
				'desc'    => __( 'Allow vendor to update product cashback', 'onplay-wallet' ),
				'type'    => 'switcher',
				'default' => 'off',
				'tooltip' => __( 'Checking this will enable sellers to change the product cashback option. If unchecked, only admin can change product cashback.', 'onplay-wallet' ),
			);
		...
		public function dokan_product_edit_after_options( $post_id ) {
			...
			<div class="dokan-cashback-options dokan-edit-row dokan-clearfix" style="margin-bottom: 20px;">
				<div class="dokan-section-heading" data-togglehandler="dokan_cashback_options">
					<h2><i class="fas fa-wallet" aria-hidden="true"></i> <?php esc_html_e( 'Cashback Options', 'onplay-wallet' ); ?></h2>
					<p><?php esc_html_e( 'Set product cashback options', 'onplay-wallet' ); ?></p>
				...
				<div class="dokan-section-content">
					...
					<div class="dokan-form-group content-half-part">
						<label for="_cashback_type" class="form-label"><?php esc_html_e( 'Cashback type', 'onplay-wallet' ); ?></label>
						<?php
					...
					<div class="dokan-form-group content-half-part">
						<label for="_cashback_amount" class="form-label"><?php esc_html_e( 'Cashback Amount', 'onplay-wallet' ); ?></label>
		 ...
		 * @return bool
		 */
		public function process_onplay_wallet_cashback_callback( $process_cashback, $order ) {
