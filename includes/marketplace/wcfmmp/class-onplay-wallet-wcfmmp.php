...
if ( ! class_exists( 'Onplay_Wallet_WCFMMP' ) ) {

	class Onplay_Wallet_WCFMMP {

		/**
		 * The single instance of the class.
		 *
		 * @var Onplay_Wallet_WCFMMP
		 * @since 1.2.9
		 */
		 ...
		 * @var string
		 */
		public static $gateway_slug = 'onplay_wallet';
		...
		public function init() {
			...
			require_once ONPLAY_WALLET_ABSPATH . 'includes/marketplace/wcfmmp/class-onplay-wallet-wcfmmp-gateway.php';
		}
		...
		public function wcfm_marketplace_settings_fields_withdrawal_charges( $withdrawal_charges, $wcfm_withdrawal_options, $withdrawal_charge ) {
			...
			$payment_withdrawal_charges = array(
				'withdrawal_charge_' . self::$gateway_slug => array(
					'label'             => __( 'Wallet Charge', 'onplay-wallet' ),
					'type'              => 'multiinput',
					'name'              => 'wcfm_withdrawal_options[withdrawal_charge][' . self::$gateway_slug . ']',
					'class'             => 'withdraw_charge_block withdraw_charge_' . self::$gateway_slug,
					'label_class'       => 'wcfm_title wcfm_ele wcfm_fill_ele withdraw_charge_block withdraw_charge_' . self::$gateway_slug,
					'value'             => $withdrawal_charge_wallet,
					'custom_attributes' => array( 'limit' => 1 ),
					'options'           => array(
						'percent' => array(
							'label'       => __( 'Percent Charge(%)', 'onplay-wallet' ),
							'type'        => 'number',
						...
						'fixed'   => array(
							'label'       => __( 'Fixed Charge', 'onplay-wallet' ),
							'type'        => 'number',
						...
						'tax'     => array(
							'label'       => __( 'Charge Tax', 'onplay-wallet' ),
							'type'        => 'number',
							...
							'hints'       => __( 'Tax for withdrawal charge, calculate in percent.', 'onplay-wallet' ),
						),
					),
				),
			);
