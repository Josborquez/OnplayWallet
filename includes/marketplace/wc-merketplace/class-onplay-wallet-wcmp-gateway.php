...
if ( ! class_exists( 'WCMp_Gateway_Wallet' ) && class_exists( 'WCMp_Payment_Gateway' ) ) {
	...
	class WCMp_Gateway_Wallet extends WCMp_Payment_Gateway {
		...
		public function __construct() {
			$this->id              = 'onplay_wallet';
			$this->payment_gateway = $this->id;
			$this->enabled         = get_wcmp_vendor_settings( 'payment_method_onplay_wallet', 'payment' );
		}
			...
			if ( $this->validate_request() ) {
				if ( $this->process_wallet_payment() ) {
					...
					if ( $this->transaction_id ) {
						return array(
							'message'        => __( 'New transaction has been initiated', 'onplay-wallet' ),
							'type'           => 'success',
		...
		public function validate_request() {
			...
			if ( $this->enabled != 'Enable' ) {
				$this->message[] = array(
					'message' => __( 'Invalid payment method', 'onplay-wallet' ),
					'type'    => 'error',
				);
				return false;
			}
			if ( $this->transaction_mode != 'admin' ) {
				...
				if ( $this->get_transaction_total() > $thesold_amount ) {
					...
					$this->message[] = array(
						'message' => __( 'Minimum threshold amount to withdrawal commission is ' . $thesold_amount, 'onplay-wallet' ),
						'type'    => 'error',
					);
					return false;
				}
		...
		private function process_wallet_payment() {
			...
			$transaction_id  = onplay_wallet()->wallet->credit( $this->vendor->id, $amount_to_pay, __( 'Commission received for commission id ', 'onplay-wallet' ) . $for_commissions, array( 'for' => 'vendor_commission' ) );
			if ( $transaction_id ) {
				return true;
			}
			return false;
