...
if ( ! class_exists( 'Onplay_Wallet_Ajax' ) ) {
	/**
	 * Plugin Ajax class
	 */
	class Onplay_Wallet_Ajax {

		/**
		 * The single instance of the class.
		 *
		 * @var Onplay_Wallet_Ajax
		 * @since 1.1.10
		 */
		...
		public function __construct() {
			add_action( 'wp_ajax_onplay_wallet_order_refund', array( $this, 'onplay_wallet_order_refund' ) );
			add_action( 'wp_ajax_woocommerce_wallet_rated', array( $this, 'woocommerce_wallet_rated' ) );
			add_action( 'wp_ajax_onplay-wallet-user-search', array( $this, 'onplay_wallet_user_search' ) );
			add_action( 'wp_ajax_onplay_wallet_partial_payment_update_session', array( $this, 'onplay_wallet_partial_payment_update_session' ) );
			add_action( 'wp_ajax_onplay_wallet_refund_partial_payment', array( $this, 'onplay_wallet_refund_partial_payment' ) );
			add_action( 'wp_ajax_onplay-wallet-dismiss-promotional-notice', array( $this, 'onplay_wallet_dismiss_promotional_notice' ) );
		...
		public function onplaywallet_export_user_search() {
			...
			foreach ( $users as $user ) {
				$return[] = array(
					...
					'label' => sprintf( _x( '%1$s (%2$s)', 'user autocomplete result', 'onplay-wallet' ), $user->user_login, $user->user_email ),
					'value' => $user->ID,
				);
			}
			wp_send_json( $return );
		 ...
		 */
		public function onplay_wallet_refund_partial_payment() {
			...
			$transaction_id         = onplay_wallet()->wallet->credit( $order->get_customer_id(), $partial_payment_amount, __( 'Wallet refund #', 'onplay-wallet' ) . $order->get_order_number(), array( 'currency' => $order->get_currency( 'edit' ) ) );
			if ( $transaction_id ) {
				$response['success'] = true;
				/* translators: wallet amount */
				$order->add_order_note( sprintf( __( '%s refunded to customer wallet', 'onplay-wallet' ), wc_price( $partial_payment_amount, onplay_wallet_wc_price_args( $order->get_customer_id() ) ) ) );
				WOO_Wallet_Helper::update_order_meta_data( $order, '_onplay_wallet_partial_payment_refunded', true, false );
				WOO_Wallet_Helper::update_order_meta_data( $order, '_partial_payment_refund_id', $transaction_id );
				do_action( 'onplay_wallet_partial_order_refunded', $order_id, $transaction_id );
			}
		 ...
		 */
		public function onplay_wallet_order_refund() {
			ob_start();
			...
			try {
				...
				$refund_reason = $refund_reason ? $refund_reason : __( 'Wallet refund #', 'onplay-wallet' ) . $order->get_order_number();
				...
				if ( ! is_wp_error( $refund ) ) {
					$transaction_id = onplay_wallet()->wallet->credit( $order->get_customer_id(), $refund_amount, $refund_reason, array( 'currency' => $order->get_currency( 'edit' ) ) );
					if ( ! $transaction_id ) {
						throw new Exception( __( 'Refund not credited to customer', 'onplay-wallet' ) );
					} else {
						do_action( 'onplay_wallet_order_refunded', $order, $refund, $transaction_id );
					}
				}
		 ...
		 * Search users
		 */
		public function onplay_wallet_user_search() {
			check_ajax_referer( 'search-user', 'security' );
			$return = array();
			$term   = isset( $_POST['term'] ) ? sanitize_text_field( wp_unslash( $_POST['term'] ) ) : '';
			if ( apply_filters( 'onplay_wallet_user_search_exact_match', true ) ) {
				$user = get_user_by( apply_filters( 'onplay_wallet_user_search_by', 'email' ), $term );
		 ...
		 * @return void
		 */
		public function onplay_wallet_partial_payment_update_session() {
			if ( isset( $_POST['checked'] ) && 'true' === $_POST['checked'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				update_wallet_partial_payment_session( onplay_wallet()->wallet->get_wallet_balance( get_current_user_id(), 'edit' ) );
			} else {
		 ...
		 * @return void
		 */
		public function onplay_wallet_dismiss_promotional_notice() {
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_send_json_error( __( 'You have no permission to do that', 'onplay-wallet' ) );
			}

			if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'onplay_wallet_admin' ) ) {
				wp_send_json_error( __( 'Invalid nonce', 'onplay-wallet' ) );
			}
			update_option( '_onplay_wallet_promotion_dismissed', true );
			wp_send_json_success();
		}
		...
		public function draw_wallet_transaction_details_table() {
			check_ajax_referer( 'onplay-wallet-transactions', 'security' );
			...
			if ( $transactions ) {
				foreach ( $transactions as $transaction ) {
					$response['data'][] = apply_filters(
						'onplay_wallet_transactons_datatable_row_data',
						array(
							'id'      => $transaction->transaction_id,
							'amount'  => '<mark class="' . esc_attr( $transaction->type ) . '">' . ( 'credit' === $transaction->type ? '+' : '-' ) . wc_price( apply_filters( 'onplay_wallet_amount', $transaction->amount, $transaction->currency, $transaction->user_id ), onplay_wallet_wc_price_args( $transaction->user_id ) ) . '</mark>',
		...
		public function edit_wallet_balance_template_data() {
			check_ajax_referer( 'onplay-wallet-edit-balance-template-data', 'security' );
			...
			if ( $user_id ) {
				...
				wp_send_json_success(
					array(
						'user_id'         => $user_id,
						'current_balance' => onplay_wallet()->wallet->get_wallet_balance( $user_id ),
					)
				);
			}
			wp_send_json_error( array( 'error' => __( 'User not found', 'onplay-wallet' ) ) );
		}
	}

}
Onplay_Wallet_Ajax::instance();
