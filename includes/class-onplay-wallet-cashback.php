...
if ( ! class_exists( 'Onplay_Wallet_Cashback' ) ) {
	/**
	 * Wallet cashback class.
	 */
	class Onplay_Wallet_Cashback {
		...
		private static function init_cashback_settings() {
			self::$cashback_rule         = onplay_wallet()->settings_api->get_option( 'cashback_rule', '_wallet_settings_credit', 'cart' );
			self::$global_cashbak_type   = onplay_wallet()->settings_api->get_option( 'cashback_type', '_wallet_settings_credit', 'percent' );
			self::$global_cashbak_amount = floatval( onplay_wallet()->settings_api->get_option( 'cashback_amount', '_wallet_settings_credit', 0 ) );
			self::$max_cashbak_amount    = floatval( onplay_wallet()->settings_api->get_option( 'max_cashback_amount', '_wallet_settings_credit', 0 ) );
		}
		...
		public static function calculate_cashback( $form_cart = true, $order_id = 0, $force = false ) {
			...
			if ( 'on' !== onplay_wallet()->settings_api->get_option( 'is_enable_cashback_reward_program', '_wallet_settings_credit' ) ) {
				return 0;
			}
		...
		private static function calculate_cashback_form_cart() {
			...
			switch ( self::$cashback_rule ) {
				...
				case 'product_cat':
					...
					break;
				case 'cart':
					if ( 0 !== onplay_wallet()->settings_api->get_option( 'min_cart_amount', '_wallet_settings_credit', 10 ) && WC()->cart->get_total( 'edit' ) >= onplay_wallet()->settings_api->get_option( 'min_cart_amount', '_wallet_settings_credit', 0 ) ) {
						if ( 'percent' === self::$global_cashbak_type ) {
							$total                   = apply_filters( 'onplay_wallet_calculate_cashback_on_total', true ) ? wc()->cart->get_total( 'edit' ) : wc()->cart->get_subtotal();
			...
			return apply_filters( 'onplay_wallet_form_cart_cashback_amount', $cashback_amount );
		}
		...
		private static function calculate_cashback_form_order( $order_id = 0 ) {
			...
			switch ( self::$cashback_rule ) {
				...
				case 'product_cat':
					...
					break;
				case 'cart':
					if ( 0 !== onplay_wallet()->settings_api->get_option( 'min_cart_amount', '_wallet_settings_credit', 10 ) && $order->get_total( 'edit' ) >= onplay_wallet()->settings_api->get_option( 'min_cart_amount', '_wallet_settings_credit', 0 ) ) {
						if ( 'percent' === self::$global_cashbak_type ) {
							$total                   = apply_filters( 'onplay_wallet_calculate_cashback_on_total', true ) ? $order->get_total( 'edit' ) : $order->get_subtotal();
							$percent_cashback_amount = ( $total * self::$global_cashbak_amount ) / 100;
							if ( self::$max_cashbak_amount && $percent_cashback_amount > self::$max_cashbak_amount ) {
								$cashback_amount += self::$max_cashbak_amount;
							} else {
			...
			$cashback_amount = apply_filters( 'onplay_wallet_form_order_cashback_amount', $cashback_amount, $order_id );
		...
		public static function get_product_cashback_amount( $product, $qty = 1, $product_price = 0 ) {
			...
			return apply_filters( 'onplay_wallet_product_wise_cashback_amount', $cashback_amount, $product, $qty, $product_price );
		}
		...
		public static function get_product_category_wise_cashback_amount( $product, $qty = 1, $product_price = 0 ) {
			...
			if ( $product_price ) {
				...
				if ( ! empty( $category_wise_cashback_amounts ) ) {
					$cashback_amount += ( 'on' === onplay_wallet()->settings_api->get_option( 'allow_min_cashback', '_wallet_settings_credit', 'on' ) ) ? min( $category_wise_cashback_amounts ) : max( $category_wise_cashback_amounts );
				} elseif ( 'percent' === self::$global_cashbak_type ) {
			...
			return apply_filters( 'onplay_wallet_product_category_wise_cashback_amount', $cashback_amount, $product, $qty, $product_price );
		}
	}
