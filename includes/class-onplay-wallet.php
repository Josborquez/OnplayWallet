...
final class Onplay_Wallet {
	 ...
	 * Wallet instance.
	 *
	 * @var Onplay_Wallet_Wallet
	 */
	public $wallet = null;
	/**
	 * Cashback instance.
	 *
	 * @var Onplay_Wallet_Cashback
	 */
	...
	public function __construct() {
		$this->includes();
		$this->init_hooks();
		do_action( 'onplay_wallet_loaded' );
	}
	...
	public function includes() {
		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-helper.php';
		include_once ONPLAY_WALLET_ABSPATH . 'includes/helper/onplay-wallet-util.php';
		include_once ONPLAY_WALLET_ABSPATH . 'includes/helper/onplay-wallet-update-functions.php';
		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-install.php';

		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-settings-api.php';
		$this->settings_api = new Onplay_Wallet_Settings_API();

		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-wallet.php';
		$this->wallet = new Onplay_Wallet_Wallet();

		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-cashback.php';
		$this->cashback = new Onplay_Wallet_Cashback();

		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-pos-connector.php';
		$this->pos_connector = new OnplayPOS_Connector();

		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-widgets.php';

		if ( $this->is_request( 'admin' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/export/class-onplaywallet-csv-exporter.php';
			include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-settings.php';
			include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-admin.php';
		}
		if ( $this->is_request( 'frontend' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-frontend.php';
		}
		if ( $this->is_request( 'ajax' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-ajax.php';
		}
	}
	...
	public function plugin_url() {
		return untrailingslashit( plugins_url( '/', ONPLAY_WALLET_PLUGIN_FILE ) );
	}
	...
	private function init_hooks() {
		register_activation_hook( ONPLAY_WALLET_PLUGIN_FILE, array( 'Onplay_Wallet_Install', 'install' ) );
		register_deactivation_hook( ONPLAY_WALLET_PLUGIN_FILE, array( $this, 'deactivate_plugin' ) );
		add_filter( 'plugin_action_links_' . plugin_basename( ONPLAY_WALLET_PLUGIN_FILE ), array( $this, 'plugin_action_links' ) );
		add_action( 'init', array( $this, 'init' ), 5 );
		add_action( 'widgets_init', array( $this, 'onplay_wallet_widget_init' ) );
	...
	public function init() {
		...
		include_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-payment-method.php';
		...
		$is_active = get_option( 'onplay_wallet_is_active', false );

		if ( false === $is_active ) {
			update_option( 'onplay_wallet_is_active', true );
			flush_rewrite_rules();
			do_action( 'onplay_wallet_activated' );
		}
	}
	...
	public function add_woocommerce_query_vars( $query_vars ) {
		$query_vars['onplay-wallet'] = get_option( 'woocommerce_onplay_wallet_endpoint', 'my-wallet' );
		return $query_vars;
	}
	...
	public function deactivate_plugin() {
		delete_option( 'onplay_wallet_is_active' );
		flush_rewrite_rules();
		do_action( 'onplay_wallet_deactivated' );
	}
	/**
	 * WooWallet init widget
	 */
	public function onplay_wallet_widget_init() {
		register_widget( 'Onplay_Wallet_Topup' );
	}
	/**
	...
	public function woocommerce_loaded_callback() {
		include_once ONPLAY_WALLET_ABSPATH . 'includes/abstracts/abstract-onplay-wallet-actions.php';
		require_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-actions.php';
		include_once ONPLAY_WALLET_ABSPATH . '/includes/class-onplay-wallet-api.php';
	...
	public function ajax_pos_test_connection() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'onplay-wallet' ) ) );
		}

		if ( ! $this->pos_connector->is_outbound_configured() ) {
			wp_send_json_error( array( 'message' => __( 'POS API URL and API Key are not configured.', 'onplay-wallet' ) ) );
		}
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Connection successful. Status: %1$s. Version: %2$s', 'onplay-wallet' ),
					$status,
					$version
				),
				'data'    => $result,
	...
	public function plugin_action_links( $links ) {
		$action_links = array(
			'settings' => '<a href="' . admin_url( 'admin.php?page=onplay-wallet-settings' ) . '" aria-label="' . esc_attr__( 'View Wallet settings', 'onplay-wallet' ) . '">' . esc_html__( 'Settings', 'onplay-wallet' ) . '</a>',
		);
	...
	public function load_plugin_textdomain() {
		...
		$locale = apply_filters( 'plugin_locale', $locale, 'onplay-wallet' );

		unload_textdomain( 'onplay-wallet', true );
		load_textdomain( 'onplay-wallet', WP_LANG_DIR . '/onplay-wallet/onplay-wallet-' . $locale . '.mo' );
		load_plugin_textdomain( 'onplay-wallet', false, plugin_basename( dirname( ONPLAY_WALLET_PLUGIN_FILE ) ) . '/languages' );
	}
	...
	public function woocommerce_email_classes( $emails ) {
		$emails['Onplay_Wallet_Email_New_Transaction']    = include ONPLAY_WALLET_ABSPATH . 'includes/emails/class-onplay-wallet-email-new-transaction.php';
		$emails['Onplay_Wallet_Email_Low_Wallet_Balance'] = include ONPLAY_WALLET_ABSPATH . 'includes/emails/class-onplay-wallet-email-low-wallet-balance.php';
		return $emails;
	}
	...
	public function woocommerce_reports_get_order_report_query( $query ) {
		...
		if ( ! empty( $rechargable_orders ) && apply_filters( 'onplay_wallet_exclude_wallet_rechargeable_orders_from_report', true ) ) {
	...
	public function remove_wallet_rechargable_order_from_analytics( $query_vars ) {
		if ( get_wallet_rechargeable_product() && apply_filters( 'onplay_wallet_exclude_wallet_rechargeable_orders_from_report', true ) ) {
	...
	public function add_marketplace_support() {
		if ( class_exists( 'WCMp' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/marketplace/wc-merketplace/class-onplay-wallet-wcmp-gateway.php';
			include_once ONPLAY_WALLET_ABSPATH . 'includes/marketplace/wc-merketplace/class-onplay-wallet-wcmp.php';
		}
		if ( class_exists( 'WeDevs_Dokan' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/marketplace/dokan/class-onplay-wallet-dokan.php';
		}
		if ( class_exists( 'WCFMmp' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/marketplace/wcfmmp/class-onplay-wallet-wcfmmp.php';
		}
	}
	/**
	...
	public function add_multicurrency_support() {
		if ( class_exists( 'WOOCS' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/multicurrency/woocommerce-currency-switcher/class-wallet-multi-currency.php';
		}
		if ( class_exists( 'WCML_Multi_Currency' ) ) {
			include_once ONPLAY_WALLET_ABSPATH . 'includes/multicurrency/woocommerce-multilingual/class-wallet-wpml-multi-currency.php';
		}
	}
	/**
	...
	public function delete_user_transaction_records( $id ) {
		global $wpdb;
		if ( apply_filters( 'onplay_wallet_delete_transaction_records', true ) ) {
			$wpdb->query( $wpdb->prepare( "DELETE t.*, tm.* FROM {$wpdb->base_prefix}onplay_wallet_transactions t JOIN {$wpdb->base_prefix}onplay_wallet_transaction_meta tm ON t.transaction_id = tm.transaction_id WHERE t.user_id = %d", $id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		}
	}
	...
	public static function add_woocommerce_block_support() {
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
			require_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-payments-blocks.php';
			add_action(
		...
		require_once ONPLAY_WALLET_ABSPATH . 'includes/class-onplay-wallet-partial-payment-blocks.php';
		add_action(
	...
	public function locate_template( $template_name, $template_path = '', $default_path = '' ) {
		$default_path = apply_filters( 'onplay_wallet_template_path', $default_path );
		if ( ! $template_path ) {
			$template_path = 'onplay-wallet';
		}
		if ( ! $default_path ) {
			$default_path = ONPLAY_WALLET_ABSPATH . 'templates/';
		}
		// Look within passed path within the theme - this is priority.
		$template = locate_template( array( trailingslashit( $template_path ) . $template_name, $template_name ) );
		// Add support of third perty plugin.
		$template = apply_filters( 'onplay_wallet_locate_template', $template, $template_name, $template_path, $default_path );
