 ...
 * DB update 108.
 */
function onplay_wallet_update_108_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
 ...
 * DB update 110.
 */
function onplay_wallet_update_110_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
 ...
 * DB update 117.
 */
function onplay_wallet_update_117_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
 ...
 * DB update 1310.
 */
function onplay_wallet_update_1310_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
 ...
 * DB update 1312.
 */
function onplay_wallet_update_1312_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
 ...
 * DB update 1321.
 */
function onplay_wallet_update_1321_db_column() {
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'onplay_wallet_transactions';
	if ( $table_name === $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table_name ) ) && $wpdb->get_var( $wpdb->prepare( 'SHOW COLUMNS FROM `%s` LIKE `amount`;', $table_name ) ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$wpdb->query( $wpdb->prepare( 'ALTER TABLE %s MODIFY COLUMN `amount` decimal(16,8);', $table_name ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
