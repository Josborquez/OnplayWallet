...
if ( ! function_exists( 'is_wallet_rechargeable_order' ) ) {
	...
	function is_wallet_rechargeable_order( $order ) {
		...
		return apply_filters( 'onplay_wallet_is_wallet_rechargeable_order', $is_wallet_rechargeable_order, $order );
	}
}
...
if ( ! function_exists( 'is_wallet_rechargeable_cart' ) ) {
	...
	function is_wallet_rechargeable_cart() {
		...
		return apply_filters( 'onplay_wallet_is_wallet_rechargeable_cart', $is_wallet_rechargeable_cart );
	}
}
...
if ( ! function_exists( 'get_onplay_wallet_cart_fee_total' ) ) {
	/**
	 * Get total fee amount from cart.
	 *
	 * @return float
	 */
	function get_onplay_wallet_cart_fee_total() {
		$fee_amount = 0;
...
if ( ! function_exists( 'get_woowallet_cart_total' ) ) {
	...
	function get_woowallet_cart_total() {
		...
		if ( ! is_admin() && is_array( wc()->cart->cart_contents ) && count( wc()->cart->cart_contents ) > 0 ) {
			$cart_total = wc()->cart->get_subtotal( 'edit' ) + wc()->cart->get_taxes_total() + wc()->cart->get_shipping_total( 'edit' ) - wc()->cart->get_discount_total() + get_woowallet_coupon_cashback_amount() + get_onplay_wallet_cart_fee_total();
		}
...
if ( ! function_exists( 'is_enable_wallet_partial_payment' ) ) {
	...
	function is_enable_wallet_partial_payment() {
		...
		if ( ! is_wallet_rechargeable_cart() && is_user_logged_in() && ( ( ! is_null( wc()->session ) && wc()->session->get( 'partial_payment_amount', false ) ) || 'on' === onplay_wallet()->settings_api->get_option( 'is_auto_deduct_for_partial_payment', '_wallet_settings_general' ) ) && $cart_total >= apply_filters( 'onplay_wallet_partial_payment_amount', onplay_wallet()->wallet->get_wallet_balance( get_current_user_id(), 'edit' ) ) ) {
			$is_enable = true;
		}
...
if ( ! function_exists( 'get_order_partial_payment_amount' ) ) {
	...
	function get_order_partial_payment_amount( $order_id ) {
		...
		return apply_filters( 'onplay_wallet_order_partial_payment_amount', abs( $via_wallet ), $order_id );
	}
}
...
if ( ! function_exists( 'get_wallet_rechargeable_product' ) ) {
	...
	function get_wallet_rechargeable_product() {
		Onplay_Wallet_Install::cteate_product_if_not_exist();
		return wc_get_product( apply_filters( 'onplay_wallet_rechargeable_product_id', get_option( '_onplay_wallet_recharge_product' ) ) );
	}
}
...
if ( ! function_exists( 'set_wallet_transaction_meta' ) ) {
	...
	function set_wallet_transaction_meta( $transaction_id, $meta_key, $meta_value, $user_id = '' ) {
		...
		$wpdb->insert( // @codingStandardsIgnoreLine
			"{$wpdb->base_prefix}onplay_wallet_transaction_meta",
			array(
		...
		clear_onplay_wallet_cache( $user_id );
		return $meta_id;
	}
}
...
if ( ! function_exists( 'update_wallet_transaction_meta' ) ) {
	...
	function update_wallet_transaction_meta( $transaction_id, $meta_key, $meta_value, $user_id = '' ) {
		global $wpdb;
		if ( is_null( $wpdb->get_var( $wpdb->prepare( "SELECT meta_id FROM {$wpdb->base_prefix}onplay_wallet_transaction_meta WHERE transaction_id = %s AND meta_key = %s", array( $transaction_id, $meta_key ) ) ) ) ) { // @codingStandardsIgnoreLine
...
if ( ! function_exists( 'get_wallet_transaction_meta' ) ) {
	...
	function get_wallet_transaction_meta( $transaction_id, $meta_key, $default = false ) {
		global $wpdb;
		$resualt = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM {$wpdb->base_prefix}onplay_wallet_transaction_meta WHERE transaction_id = %s AND meta_key = %s", $transaction_id, $meta_key ) ); // @codingStandardsIgnoreLine
		if ( ! is_null( $resualt ) ) {
...
if ( ! function_exists( 'get_wallet_transactions' ) ) {
	...
	function get_wallet_transactions( $args = array(), $output = OBJECT ) {
		...
		$args         = apply_filters( 'onplay_wallet_transactions_query_args', $args );
		$args         = wp_parse_args( $args, $default_args );
		extract( $args ); // @codingStandardsIgnoreLine
		// Build query safely: validate identifiers and use prepared statements for values.
		$select = 'SELECT transactions.*';
		$from   = "FROM {$wpdb->base_prefix}onplay_wallet_transactions AS transactions";
		...
		$joins = array();
		if ( ! empty( $where_meta ) ) {
			$joins[] = "{$join_type} JOIN {$wpdb->base_prefix}onplay_wallet_transaction_meta AS transaction_meta ON transactions.transaction_id = transaction_meta.transaction_id";
		}
		...
		$query = apply_filters( 'onplay_wallet_transactions_query', array( $select, $from, implode( ' ', $joins ), 'WHERE ' . implode( ' AND ', $where_clauses ), $order_by_sql, $limit_sql ) );
		$query = implode( ' ', $query );

		$query_hash     = md5( absint( $user_id ) . $query );
		$cached_results = is_array( get_transient( "onplay_wallet_transaction_resualts_{$user_id}" ) ) ? get_transient( "onplay_wallet_transaction_resualts_{$user_id}" ) : array();
		...
		if ( $nocache || ! isset( $cached_results[ $query_hash ] ) ) {
			...
			if ( 'all_with_meta' === $fields ) {
				foreach ( $query_resualts as $key => $query_resualt ) {
					$query_resualts[ $key ]->meta = $wpdb->get_results( $wpdb->prepare( "SELECT transaction_meta.meta_key, transaction_meta.meta_value FROM {$wpdb->base_prefix}onplay_wallet_transaction_meta AS transaction_meta WHERE transaction_id = %d", $query_resualt->transaction_id ), $output ); // @codingStandardsIgnoreLine
				}
			}
			$cached_results[ $query_hash ] = $query_resualts;
			set_transient( "onplay_wallet_transaction_resualts_{$user_id}", $cached_results, DAY_IN_SECONDS );
		}
...
if ( ! function_exists( 'get_wallet_transactions_count' ) ) {
	...
	function get_wallet_transactions_count( $user_id = null, $include_deleted = false ) {
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->base_prefix}onplay_wallet_transactions";
		if ( ! empty( $where ) ) {
...
if ( ! function_exists( 'get_wallet_transaction' ) ) {
	...
	function get_wallet_transaction( $transaction_id ) {
		global $wpdb;
		$transaction = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->base_prefix}onplay_wallet_transactions WHERE transaction_id = %d", $transaction_id ) ); // @codingStandardsIgnoreLine
		return $transaction;
	}
}
...
if ( ! function_exists( 'get_wallet_transaction_type' ) ) {
	...
	function get_wallet_transaction_type( $transaction_id ) {
		global $wpdb;
		$transaction = $wpdb->get_row( $wpdb->prepare( "SELECT type FROM {$wpdb->base_prefix}onplay_wallet_transactions WHERE transaction_id = %d", $transaction_id ) ); // @codingStandardsIgnoreLine
		if ( $transaction ) {
...
if ( ! function_exists( 'update_wallet_transaction' ) ) {
	...
	function update_wallet_transaction( $transaction_id, $user_id, $data = array(), $format = null ) {
		...
		$update = false;
		if ( ! empty( $data ) ) {
			$update = $wpdb->update( "{$wpdb->base_prefix}onplay_wallet_transactions", $data, array( 'transaction_id' => $transaction_id ), $format, array( '%d' ) ); // @codingStandardsIgnoreLine
			if ( $update ) {
				clear_onplay_wallet_cache( $user_id );
			}
		}
...
if ( ! function_exists( 'clear_onplay_wallet_cache' ) ) {
	 ...
	 * @param int $user_id user_id.
	 */
	function clear_onplay_wallet_cache( $user_id = '' ) {
		if ( ! $user_id ) {
			$user_id = get_current_user_id();
		}

		delete_transient( "onplay_wallet_transaction_resualts_{$user_id}" );
	}
}
...
if ( ! function_exists( 'get_wallet_cashback_amount' ) ) {
	...
	function get_wallet_cashback_amount( $order_id = 0 ) {
		_deprecated_function( 'get_wallet_cashback_amount', '1.3.0', 'onplay_wallet()->cashback->calculate_cashback()' );
		if ( $order_id ) {
			return onplay_wallet()->cashback->calculate_cashback( false, $order_id );
		}
		return onplay_wallet()->cashback->calculate_cashback();
	}
}
...
if ( ! function_exists( 'is_full_payment_through_wallet' ) ) {
	...
	function is_full_payment_through_wallet() {
		...
		$current_wallet_balance          = onplay_wallet()->wallet->get_wallet_balance( get_current_user_id(), 'edit' );
		$total                           = 0;
		$order_id                        = null;
		if ( WC()->cart ) {
...
if ( ! function_exists( 'get_total_order_cashback_amount' ) ) {
	...
	function get_total_order_cashback_amount( $order_id ) {
		...
		return apply_filters( 'onplay_wallet_total_order_cashback_amount', $total_cashback_amount );
	}
}

if ( ! function_exists( 'onplay_wallet_persistent_cart_update' ) ) {
	/**
	 * Update WooWallet persistent cart to restore cart after recharge wallet.
	 */
	function onplay_wallet_persistent_cart_update() {
		if ( get_current_user_id() && apply_filters( 'onplay_wallet_persistent_cart_enabled', true ) ) {
			update_user_meta(
				get_current_user_id(),
				'_onplay_wallet_persistent_cart_' . get_current_blog_id(),
...
if ( ! function_exists( 'onplay_wallet_persistent_cart_destroy' ) ) {
	/**
	 * Delete WooWallet persistent cart after restoring WooCommerce cart.
	 */
	function onplay_wallet_persistent_cart_destroy() {
		if ( get_current_user_id() ) {
			delete_user_meta( get_current_user_id(), '_onplay_wallet_persistent_cart_' . get_current_blog_id() );
		}
	}
}

if ( ! function_exists( 'onplay_wallet_get_saved_cart' ) ) {
	/**
	 * Get saved WooWallet cart items.
	 *
	 * @return array
	 */
	function onplay_wallet_get_saved_cart() {
		$saved_cart = array();

		if ( apply_filters( 'onplay_wallet_persistent_cart_enabled', true ) ) {
			$saved_cart_meta = get_user_meta( get_current_user_id(), '_onplay_wallet_persistent_cart_' . get_current_blog_id(), true );
...
if ( ! function_exists( 'onplay_wallet_wc_price_args' ) ) {
	/**
	 * Get WC price args.
	 *
	 * @param int $user_id user_id.
	 * @return array
	 */
	function onplay_wallet_wc_price_args( $user_id = '', $args = array() ) {
		if ( ! $user_id ) {
			$user_id = get_current_user_id();
		}
		$args = apply_filters(
			'onplay_wallet_wc_price_args',
			wp_parse_args(
				$args,
				array(
					'ex_tax_label'       => false,
...
if ( ! function_exists( 'get_wallet_user_capability' ) ) {
	...
	function get_wallet_user_capability() {
		return apply_filters( 'onplay_wallet_user_capability', 'manage_woocommerce' );
	}
}
...
if ( ! function_exists( 'delete_user_wallet_transactions' ) ) {
	...
	function delete_user_wallet_transactions( $user_id, $force_delete = false ) {
		global $wpdb;
		if ( ! $force_delete ) {
			$update = $wpdb->update( "{$wpdb->base_prefix}onplay_wallet_transactions", array( 'deleted' => 1 ), array( 'user_id' => $user_id ), array( '%d' ), array( '%d' ) ); // @codingStandardsIgnoreLine
			if ( $update ) {
				clear_onplay_wallet_cache( $user_id );
			}
		} else {
			...
			if ( $user_wallet_transactions ) {
				foreach ( $user_wallet_transactions as $transaction ) {
					$wpdb->delete( "{$wpdb->base_prefix}onplay_wallet_transactions", array( 'transaction_id' => $transaction->transaction_id ) ); // @codingStandardsIgnoreLine
					$wpdb->delete( "{$wpdb->base_prefix}onplay_wallet_transaction_meta", array( 'transaction_id' => $transaction->transaction_id ) ); // @codingStandardsIgnoreLine
				}
			}
			clear_onplay_wallet_cache( $user_id );
		}
...
if ( ! function_exists( 'is_wallet_account_locked' ) ) {
	...
	function is_wallet_account_locked( $user_id = '' ) {
		...
		return apply_filters( 'onplay_wallet_is_user_wallet_locked', get_user_meta( $user_id, '_is_wallet_locked', true ), $user_id );
	}
}
