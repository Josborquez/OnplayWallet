...
if ( ! class_exists( 'WooWallet_API' ) ) {
	...
	class WooWallet_API {
		...
		private function rest_api_includes() {
			include_once dirname( __FILE__ ) . '/api/class-wc-rest-onplay-wallet-controller.php';
		...
		public function register_rest_routes() {
			...
			$controllers = array(
				// v2 controllers.
				'WC_REST_Onplay_Wallet_Controller',
				// v3 controllers.
				'WC_REST_OnplayWallet_V3_Controller',
			);
			foreach ( $controllers as $controller ) {
				$onplay_wallet_api = new $controller();
				$onplay_wallet_api->register_routes();
			}
		}
