...
class WOO_Wallet_REST_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		register_rest_route(
			...
			array(
				'args'   => array(
					'id' => array(
						'description' => __( 'Unique identifier for the resource.', 'onplay-wallet' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_item( $request ) {
		...
		$balance = onplay_wallet()->wallet->get_wallet_balance( $params['id'], 'edit' );
	...
	public function create_item( $request ) {
		...
		if ( isset( $params['type'] ) && isset( $params['amount'] ) ) {
			...
			if ( 'credit' === $params['type'] ) {
				$transaction_id = onplay_wallet()->wallet->credit( $params['id'], $params['amount'], $params['details'] );
			} elseif ( 'debit' === $params['type'] ) {
				$transaction_id = onplay_wallet()->wallet->debit( $params['id'], $params['amount'], $params['details'] );
			}
	...
	public function get_items_permissions_check( $request ) {
		return apply_filters( 'onplay_wallet_rest_check_permissions', current_user_can( 'manage_woocommerce' ), 'read', $request );
	}
	...
	public function create_item_permissions_check( $request ) {
		return apply_filters( 'onplay_wallet_rest_check_permissions', current_user_can( 'manage_woocommerce' ), 'create', $request );
	}
