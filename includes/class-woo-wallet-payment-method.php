...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	class Woo_Gateway_Wallet_payment extends WC_Payment_Gateway {
		...
		public function is_available() {
			if ( is_checkout() ) {
				$connector = woo_wallet()->pos_connector;
				...
				if ( $connector->is_ssot_enabled() && $connector->is_outbound_configured() ) {
					$cache_balance = floatval( get_user_meta( get_current_user_id(), '_current_woo_wallet_balance', true ) );
					$cart_total    = 0;
					if ( WC()->cart ) {
						$cart_total = floatval( WC()->cart->get_total( 'edit' ) );
					}
					$has_balance = ( $cache_balance >= $cart_total && $cart_total > 0 );
					return apply_filters( 'woo_wallet_payment_is_available', ( parent::is_available() && $has_balance && is_user_logged_in() && ! is_enable_wallet_partial_payment() && ! is_wallet_account_locked() ) );
				}
		...
		public function get_icon() {
			...
			if ( $connector->is_ssot_enabled() ) {
				$cache_balance = floatval( get_user_meta( get_current_user_id(), '_current_woo_wallet_balance', true ) );
				$current_balance = wc_price( $cache_balance, woo_wallet_wc_price_args( get_current_user_id() ) );
			} else {
		...
		private function process_payment_pos_ssot( $order, $connector ) {
			...
			if ( is_wp_error( $balance_response ) ) {
				...
				wc_add_notice( __( 'Error al verificar saldo. Intenta nuevamente.', 'woo-wallet' ), 'error' );
			...
			if ( $pos_balance < $amount ) {
				wc_add_notice(
					sprintf(
						...
						__( 'Saldo insuficiente. Disponible: %s', 'woo-wallet' ),
			...
			$description = sprintf(
				...
				__( 'Compra en %1$s - Orden #%2$s', 'woo-wallet' ),
				get_bloginfo( 'name' ),
			...
			if ( is_wp_error( $debit_response ) ) {
				...
				wc_add_notice( __( 'Error al procesar el pago. Intenta nuevamente.', 'woo-wallet' ), 'error' );
				return array( 'result' => 'failure' );
			}
			...
			if ( $user ) {
				...
				update_user_meta( $user->ID, '_current_woo_wallet_balance', $new_balance );
			}
			...
			$order->add_order_note(
				sprintf(
					...
					__( 'Pago wallet via POS SSoT. Monto: %1$s. Ref: %2$s', 'woo-wallet' ),
					wc_price( $amount ),
					$reference
				)
			);
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			...
			if ( $connector->is_ssot_enabled() && $connector->is_outbound_configured() && $order->get_meta( '_pos_ssot_payment' ) ) {
				...
				$description = sprintf(
					...
					__( 'Reembolso orden #%1$s en %2$s', 'woo-wallet' ),
					$order->get_order_number(),
					get_bloginfo( 'name' )
				);
				if ( $reason ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					throw new Exception( __( 'Error al devolver saldo al POS: ', 'woo-wallet' ) . $result->get_error_message() );
				}

				// Update local cache.
				$user = $order->get_user();
				if ( $user && isset( $result['balance'] ) ) {
					update_user_meta( $user->ID, '_current_woo_wallet_balance', floatval( $result['balance'] ) );
				}

				$order->add_order_note(
					sprintf(
						/* translators: %s: reference */
						__( 'Crédito devuelto al wallet POS. Ref: %s', 'woo-wallet' ),
						$reference
					)
				);

				do_action( 'woo_wallet_order_refunded', $order, $amount, isset( $result['transaction_id'] ) ? $result['transaction_id'] : 0 );
				return true;
			}
