...
class Onplay_Wallet_Transaction_Details extends WP_List_Table {
	...
	public function get_columns() {
		return apply_filters(
			'manage_onplay_wallet_transactions_columns',
			array(
				'name'           => __( 'Name', 'onplay-wallet' ),
				'type'           => __( 'Type', 'onplay-wallet' ),
				'amount'         => __( 'Amount', 'onplay-wallet' ),
				'details'        => __( 'Details', 'onplay-wallet' ),
				'created_by'     => __( 'Created By', 'onplay-wallet' ),
	...
	public function no_items() {
		esc_html_e( 'No transactions found.', 'onplay-wallet' );
	}
	...
	private function table_data( $lower = 0, $uper = 10 ) {
		...
		$this->total_count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->base_prefix}onplay_wallet_transactions WHERE user_id=%d", $user_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		if ( ! empty( $transactions ) && is_array( $transactions ) ) {
			foreach ( $transactions as $key => $transaction ) {
				$data[] = array(
					'transaction_id' => $transaction->transaction_id,
					'name'           => get_user_by( 'ID', $transaction->user_id )->display_name,
					'type'           => ( 'credit' === $transaction->type ) ? __( 'Credit', 'onplay-wallet' ) : __( 'Debit', 'onplay-wallet' ),
					'amount'         => wc_price( apply_filters( 'onplay_wallet_amount', $transaction->amount, $transaction->currency, $transaction->user_id ), onplay_wallet_wc_price_args( $transaction->user_id, array( 'currency' => $transaction->currency ) ) ),
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			default:
				return apply_filters( 'onplay_wallet_transaction_details_column_default', print_r( $item, true ), $column_name, $item ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
	}
	/**
