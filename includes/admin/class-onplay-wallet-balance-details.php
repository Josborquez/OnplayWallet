 ...
 */
class Onplay_Wallet_Balance_Details extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			'cb'             => __( 'cb', 'onplay-wallet' ),
			'username'       => __( 'Username', 'onplay-wallet' ),
			'email'          => __( 'Email', 'onplay-wallet' ),
			'total_deposits' => __( 'Total Deposits', 'onplay-wallet' ),
			'total_spent'    => __( 'Total Spent', 'onplay-wallet' ),
			'cashbak_earned' => __( 'Cashback Earned', 'onplay-wallet' ),
			'balance'        => __( 'Remaining balance', 'onplay-wallet' ),
			'status'         => __( 'Status', 'onplay-wallet' ),
			'id'             => __( 'ID', 'onplay-wallet' ),
		);
		if ( 'off' === onplay_wallet()->settings_api->get_option( 'is_enable_cashback_reward_program', '_wallet_settings_credit', 'off' ) ) {
			unset( $columns['cashbak_earned'] );
		}
		return apply_filters( 'onplay_wallet_balance_details_columns', $columns );
	}
	...
	public function no_items() {
		esc_html_e( 'No users found.', 'onplay-wallet' );
	}
	/**
	...
	public function prepare_items() {
		...
		if ( isset( $args['orderby'] ) ) {
			if ( 'balance' === $args['orderby'] ) {
				$args = array_merge(
					$args,
					array(
						'meta_key' => '_current_onplay_wallet_balance', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
						'orderby'  => 'meta_value_num',
					)
				);
			}
		}
		$args = apply_filters( 'onplay_wallet_users_list_table_query_args', $args );
		...
		foreach ( $wp_user_search->get_results() as $user ) {
			$data[] = apply_filters(
				'onplay_wallet_balance_details_list_table_item_data',
				array(
					'id'       => $user->ID,
					'username' => $user->data->user_login,
					'name'     => $user->data->display_name,
					'email'    => $user->data->user_email,
					'balance'  => onplay_wallet()->wallet->get_wallet_balance( $user->ID ),
					'actions'  => '',
				),
				$user
			);
	...
	protected function get_views() {
		...
		$url = 'admin.php?page=onplay-wallet';
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			...
			echo( sprintf( "<label class='alignleft actions bulkactions'>%s(%s): <input name='amount' type='number' step='0.01' id='amount'></input></label>", esc_html__( 'Amount', 'onplay-wallet' ), get_woocommerce_currency_symbol() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo( sprintf( "<label class='alignleft actions bulkactions'>%s: <input name='description' type='text' id='description'></input></label>", esc_html__( 'Description', 'onplay-wallet' ) ) );
		}
		do_action( 'onplay_wallet_users_list_extra_tablenav', $which );
	}
	...
	public function get_sortable_columns() {
		...
		return apply_filters( 'onplay_wallet_balance_details_sortable_columns', $sortable_columns );
	}
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			default:
				return apply_filters( 'onplay_wallet_balance_details_column_default', esc_html( print_r( $item, true ) ), $column_name, $item ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
	}
	/**
	...
	protected function column_username( $item ) {
		...
		$edit_balance_link = add_query_arg(
			array(
				...
				'security'  => wp_create_nonce( 'onplay-wallet-edit-balance-template' ),
				'user_id'   => $item['id'],
				'TB_iframe' => false,
				'width'     => 450,
				'height'    => 450,
		...
		// Get actions.
		$actions = array(
			'edit' => '<a href="#" class="edit-wallet-balance" data-user-id="' . $user_object->ID . '">' . esc_html__( 'Edit Balance', 'onplay-wallet' ) . '</a>',
		);
	...
	protected function column_total_deposits( $item ) {
		...
		return wc_price( $total_deposits, onplay_wallet_wc_price_args() );
	}
	/**
	...
	protected function column_total_spent( $item ) {
		...
		return wc_price( $total_spent_by_wallet + $total_partial_payment, onplay_wallet_wc_price_args() );
	}
	/**
	...
	protected function column_cashbak_earned( $item ) {
		...
		return wc_price( $total_cashback, onplay_wallet_wc_price_args() );
	}
	/**
	...
	protected function column_status( $item ) {
		...
		<span style="color: <?php echo $is_locked ? '#D63638' : '#135E96'; ?>;">
			<?php if ( $is_locked ) { ?>
				<span class="dashicons dashicons-lock" title="<?php echo esc_attr( __( 'Locked', 'onplay-wallet' ) ); ?>"></span>
			<?php } else { ?>
				<span class="dashicons dashicons-yes" title="<?php echo esc_attr( __( 'Active', 'onplay-wallet' ) ); ?>"></span>
			<?php } ?>
		</span>
		<?php
	}
	...
	private function process_bulk_actions() {
		if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'bulk-transactions' ) ) {
			if ( 'credit' === $this->current_action() ) {
				...
				if ( $amount && $credit_ids ) {
					foreach ( $credit_ids as $id ) {
						onplay_wallet()->wallet->credit( $id, $amount, $description );
					}
				}
				header( 'Refresh: 0' );
			}
			...
			if ( 'debit' === $this->current_action() ) {
				...
				if ( $amount && $debit_ids ) {
					foreach ( $debit_ids as $id ) {
						onplay_wallet()->wallet->debit( $id, $amount, $description );
					}
				}
				header( 'Refresh: 0' );
			}
			...
			if ( 'delete_log' === $this->current_action() ) {
				...
				if ( $delete_ids ) {
					foreach ( $delete_ids as $id ) {
						$current_balance = onplay_wallet()->wallet->get_wallet_balance( $id, 'edit' );
						delete_user_wallet_transactions( $id, true );
						if ( $current_balance && apply_filters( 'onplay_wallet_credit_user_after_delete_log', true ) ) {
							onplay_wallet()->wallet->credit( $id, $current_balance, __( 'Balance after deleting transaction logs', 'onplay-wallet' ) );
						}
					}
				}
				header( 'Refresh: 0' );
			...
			do_action( 'onplay_wallet_balance_details_process_bulk_actions', $this->current_action() );
		}
	}
	/**
	 * Get bulk options.
	...
	protected function get_bulk_actions() {
		$actions = apply_filters(
			'onplay_wallet_balance_details_bulk_actions',
			array(
				'credit'     => __( 'Credit', 'onplay-wallet' ),
				'debit'      => __( 'Debit', 'onplay-wallet' ),
				'delete_log' => __( 'Delete Log', 'onplay-wallet' ),
			)
		);
		return $actions;
	}
	...
	public function add_js_scripts() {
		...
		if ( 'toplevel_page_onplay-wallet' === $screen->id ) {
			ob_start();
			onplay_wallet()->get_template( 'admin/edit-balance.php' );
			echo ob_get_clean(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			wp_enqueue_script( 'wc-backbone-modal' );
		}

		$bulk_delete_log_msg = __( 'You are about to delete transaction records from database for selected users.', 'onplay-wallet' );
		?>
		<script type="text/javascript">
			jQuery(function ($) {
				$('.toplevel_page_onplay-wallet #posts-filter').submit(function(){
					if($('[name="action"]').val()=='delete_log' || $('[name="action2"]').val()=='delete_log'){
						return confirm('<?php echo esc_html( $bulk_delete_log_msg ); ?>');
					}
					return true;
				});
				$(document).on('click', '.toplevel_page_onplay-wallet .edit-wallet-balance', function (event) {
					event.preventDefault();
					var self = $(this);
					self.html('<?php echo esc_js( __( 'Loading...', 'onplay-wallet' ) ); ?>');
					...
					$.ajax({
						...
						data:    {
							...
							security: '<?php echo esc_js( wp_create_nonce( 'onplay-wallet-edit-balance-template-data' ) ); ?>'
						},
						type:    'GET',
						success: function( response ) {
							if ( response.success ) {
								$( this ).WCBackboneModal({
									template: 'onplay-wallet-modal-edit-balance',
									variable : response.data
								});
							}
							self.html('<?php echo esc_js( __( 'Edit Balance', 'onplay-wallet' ) ); ?>');
						}
					});
				});
			});
