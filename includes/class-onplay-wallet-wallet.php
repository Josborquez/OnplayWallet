...
if ( ! class_exists( 'Onplay_Wallet_Wallet' ) ) {

	class Onplay_Wallet_Wallet {
		/**
		 * WordPress user ID.
		 *
		 * @var INT
		 ...
		 * @var string
		 */
		public $meta_key = '_current_onplay_wallet_balance';
		...
		public function get_wallet_balance( $user_id = '', $context = 'view' ) {
			...
			if ( $this->user_id ) {
				$this->wallet_balance = $wpdb->get_var( $wpdb->prepare( "SELECT SUM(CASE WHEN t.type = 'credit' THEN t.amount ELSE -t.amount END) as balance FROM {$wpdb->base_prefix}onplay_wallet_transactions AS t WHERE t.user_id=%d AND t.deleted=0", $this->user_id ) ); // @codingStandardsIgnoreLine
				$this->wallet_balance = (float) apply_filters( 'onplay_wallet_current_balance', $this->wallet_balance, $this->user_id );
			}
			return 'view' === $context ? wc_price( $this->wallet_balance, onplay_wallet_wc_price_args( $this->user_id ) ) : number_format( $this->wallet_balance, wc_get_price_decimals(), '.', '' );
		}
		...
		public function wallet_credit_purchase( $order_id ) {
			...
			if ( 'on' === onplay_wallet()->settings_api->get_option( 'is_enable_gateway_charge', '_wallet_settings_general', 'off' ) ) {
				$charge_amount = onplay_wallet()->settings_api->get_option( 'charge_amount_' . $order->get_payment_method(), '_wallet_settings_general', 0 );
				if ( 'percent' === onplay_wallet()->settings_api->get_option( 'gateway_charge_type', '_wallet_settings_general', 'percent' ) ) {
			...
			$transaction_id = $this->credit(
				...
				$recharge_amount,
				__( 'Wallet credit through purchase #', 'onplay-wallet' ) . $order->get_order_number(),
				array(
					'for'      => 'credit_purchase',
			...
			if ( $transaction_id ) {
				...
				do_action( 'onplay_wallet_credit_purchase_completed', $transaction_id, $order );
			}
		}
		/**
		...
		public function wallet_cashback( $order_id ) {
			$order = wc_get_order( $order_id );
			/* General Cashback */
			if ( apply_filters( 'process_onplay_wallet_general_cashback', ! $order->get_meta( '_general_cashback_transaction_id' ) && $order->get_customer_id(), $order ) && onplay_wallet()->cashback->calculate_cashback( false, $order->get_id() ) ) {
				$transaction_id = $this->credit(
					$order->get_customer_id(),
					onplay_wallet()->cashback->calculate_cashback( false, $order->get_id() ),
					__( 'Wallet credit through cashback #', 'onplay-wallet' ) . $order->get_order_number(),
					array(
						'for'      => 'cashback',
						'currency' => $order->get_currency( 'edit' ),
					)
				...
				if ( $transaction_id ) {
					...
					do_action( 'onplay_wallet_general_cashback_credited', $transaction_id, $order );
				}
			}
			/* Coupon Cashback */
			if ( apply_filters( 'process_onplay_wallet_coupon_cashback', ! $order->get_meta( '_coupon_cashback_transaction_id' ) && $order->get_customer_id(), $order ) && $order->get_meta( '_coupon_cashback_amount' ) ) {
				$coupon_cashback_amount = apply_filters( 'onplay_wallet_coupon_cashback_amount', $order->get_meta( '_coupon_cashback_amount' ), $order );
				if ( $coupon_cashback_amount ) {
					$transaction_id = $this->credit(
						$order->get_customer_id(),
						$coupon_cashback_amount,
						__( 'Wallet credit through cashback by applying coupon', 'onplay-wallet' ),
						array(
							'for'      => 'cashback',
							'currency' => $order->get_currency( 'edit' ),
						)
					...
					if ( $transaction_id ) {
						...
						do_action( 'onplay_wallet_coupon_cashback_credited', $transaction_id, $order );
					}
				}
			}
		}
		...
		public function woocommerce_order_processed( $order ) {
			...
			if ( $partial_payment_amount && ! $order->get_meta( '_partial_pay_through_wallet_compleate' ) ) {
				$transaction_id = $this->debit( $order->get_customer_id(), $partial_payment_amount, __( 'For order payment #', 'onplay-wallet' ) . $order->get_order_number(), array( 'for' => 'partial_payment' ) );
				if ( $transaction_id ) {
					/* translators: wallet amount */
					$order->add_order_note( sprintf( __( '%s paid through wallet', 'onplay-wallet' ), wc_price( $partial_payment_amount, onplay_wallet_wc_price_args( $order->get_customer_id() ) ) ) );
					WOO_Wallet_Helper::update_order_meta_data( $order, '_partial_pay_through_wallet_compleate', $transaction_id );
					do_action( 'onplay_wallet_partial_payment_completed', $transaction_id, $order );
				}
			}
		...
		public function process_cancelled_order( $order_id ) {
			...
			if ( $partial_payment_amount && $order->get_meta( '_partial_pay_through_wallet_compleate' ) ) {
				...
				$this->credit( $order->get_customer_id(), $partial_payment_amount, sprintf( __( 'Your order with ID #%s has been cancelled and hence your wallet amount has been refunded!', 'onplay-wallet' ), $order->get_order_number() ), array( 'currency' => $order->get_currency( 'edit' ) ) );
				/* translators: wallet amount */
				$order->add_order_note( sprintf( __( 'Wallet amount %s has been credited to customer upon cancellation', 'onplay-wallet' ), $partial_payment_amount ) );
				$order->delete_meta_data( '_partial_pay_through_wallet_compleate' );
				WOO_Wallet_Helper::update_order_meta_data( $order, '_onplay_wallet_partial_payment_refunded', true );
			}

			/** Debit cashback amount * */
			if ( apply_filters( 'onplay_wallet_debit_cashback_upon_cancellation', get_total_order_cashback_amount( $order_id ) ) ) {
				$total_cashback_amount = get_total_order_cashback_amount( $order_id );
				if ( $total_cashback_amount ) {
					/* translators: Order number */
					if ( $this->debit( $order->get_customer_id(), $total_cashback_amount, sprintf( __( 'Cashback for #%s has been debited upon cancellation', 'onplay-wallet' ), $order->get_order_number() ) ) ) {
		...
		private function recode_transaction( $amount, $type, $details, $args = null ) {
			...
			$lock_acquired = false;
			$lock_name     = 'onplay_wallet_lock_user_' . $this->user_id;
			$lock_timeout  = apply_filters( 'onplay_wallet_db_lock_timeout', 5, $this->user_id );
			...
			try {
				...
				if ( 'debit' === $type && apply_filters( 'onplay_wallet_disallow_negative_transaction', ( $balance <= 0 || $amount > $balance ), $amount, $balance ) ) {
					return false;
				}
				if ( 'credit' === $type ) {
					$balance += $amount;
				...
				if ( $wpdb->insert( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
					"{$wpdb->base_prefix}onplay_wallet_transactions",
					apply_filters(
						'onplay_wallet_transactions_args',
						array(
					...
					clear_onplay_wallet_cache( $this->user_id );
					do_action( 'onplay_wallet_transaction_recorded', $transaction_id, $this->user_id, $amount, $type );
					$email_admin = WC()->mailer()->emails['Onplay_Wallet_Email_New_Transaction'];
					...
					$low_balance_email = WC()->mailer()->emails['Onplay_Wallet_Email_Low_Wallet_Balance'];
